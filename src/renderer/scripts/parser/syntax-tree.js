...
export class SyntaxNode {
   ...
   * @param {Map<string, string>} [sourceEditMap] - Optional map of
   *   node-id → source-edit text used by code-blocks in source view.
   * @returns {string}
   */
  toMarkdown(sourceEditMap) {
    switch (this.type) {
      case 'heading1':
        return `# ${this.content}`;
      case 'heading2':
      ...
      case 'blockquote':
        return this.content
          ...
          .join('\n');
      case 'code-block': {
        if (sourceEditMap?.has(this.id)) return sourceEditMap.get(this.id);
      ...
      case 'html-element': {
        ...
        for (const child of this.children) {
          parts.push(child.toMarkdown(sourceEditMap));
        }
        if (this.runtime.closingTag) {
          parts.push(this.runtime.closingTag);
        }
...
export class SyntaxTree {
  ...
  toMarkdown(sourceEditMap) {
    ...
    for (const child of this.children) {
      lines.push(child.toMarkdown(sourceEditMap));
    }
