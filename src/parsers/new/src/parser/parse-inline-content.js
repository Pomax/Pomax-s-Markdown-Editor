...
export function parseInlineContent(content) {
  ...
  for (const token of tokens) {
    ...
    if (
      ...
      if (idx !== -1) {
        ...
        const meta =
          /** @type {{type: string, closeType: string, raw: string, href?: string, tag?: string, attrs?: Record<string, string>}} */ (
            openStack.pop()
          );
        const children = /** @type {SyntaxNode[]} */ (stack.pop());
    ...
    if (token.type === `html-close`) {
      ...
      if (idx !== -1) {
        ...
        const children = /** @type {SyntaxNode[]} */ (stack.pop());
        const node = new SyntaxNode(`html-element`, ``);
        node.tagName = /** @type {string} */ (meta.tag);
  ...
  while (openStack.length > 0) {
    const meta = /** @type {{type: string, closeType: string, raw: string}} */ (openStack.pop());
    const children = /** @type {SyntaxNode[]} */ (stack.pop());
