 ...
 *
 * @param {SyntaxNode[]} children - Array of inline SyntaxNode instances.
 * @returns {string}
 */
 ...
 *
 * @param {import('../syntax-tree/syntax-tree.js').SyntaxTree | SyntaxNode} tree - A SyntaxTree instance.
 * @returns {string}
 */
  ...
  switch (node.type) {
    ...
    case `list`: {
      return node.children
        .map((/** @type {SyntaxNode} */ child) => renderNodeToMarkdown(child, depth))
        .join(`\n`);
    }
    case `list-item`: {
    ...
    case `table`: {
      const rows = [];
      for (const child of node.children) {
        const cells = child.children.map((/** @type {SyntaxNode} */ cell) => {
    ...
    case `html-element`: {
      ...
      if (
        ...
        const inner = node.children
          .map((/** @type {SyntaxNode} */ c) => renderNodeToMarkdown(c))
          .join(``);
