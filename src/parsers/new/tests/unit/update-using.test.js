...
import { parse, SyntaxNode, SyntaxTree } from '../../index.js';
...
describe(`SyntaxTree.updateUsing`, () => {
  it(`preserves all IDs when trees are identical`, async () => {
    const oldTree = await parse(README);
    const newTree = await parse(README);
    ...
    assert.strictEqual(await oldTree.toMarkdown(), await newTree.toMarkdown());
  });
  ...
  it(`preserves ID of an edited paragraph and updates its content`, async () => {
    ...
    const oldTree = await parse(README);
    const newTree = await parse(editedReadme);
  ...
  it(`preserves html-element child IDs during recursive diffing`, () => {
  ...
  it(`preserves every node ID on a toMarkdown-reparse round-trip with no edits`, async () => {
    ...
    const markdown = await oldTree.toMarkdown();
    const newTree = await parse(markdown);
