...
test('focused line shows raw HTML tags', async () => {
    const markdown = 'H<sub>2</sub>O is water\n\nSecond paragraph';
    await page.evaluate((content) => {
        window.editorAPI?.setContent(content);
    }, markdown);
    ...
    // Click the first line so it IS focused — should show raw syntax.
    const firstLine = page.locator('#editor .md-line').first();
    ...
    expect(text).toContain('</sub>');
    expect(text).toBe('H<sub>2</sub>O is water');

    // No actual <sub> element should exist when focused.
    expect(await firstLine.locator('sub').count()).toBe(0);
});
