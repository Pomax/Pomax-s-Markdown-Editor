...
describe("ClipboardHandler.fixHtmlTags", () => {
  ...
  it("returns the slice unchanged when all tags are balanced", () => {
    assert.equal(
      ClipboardHandler.fixHtmlTags("<strong>bold</strong>"),
      "<strong>bold</strong>",
    );
  });
  ...
  it("appends a missing closing tag", () => {
    assert.equal(
      ClipboardHandler.fixHtmlTags("<strong>in"),
      "<strong>in</strong>",
    );
  });
  ...
  it("prepends a missing opening tag", () => {
    assert.equal(
      ClipboardHandler.fixHtmlTags("nd</strong>"),
      "<strong>nd</strong>",
    );
  });
...
describe("ClipboardHandler.nodeToPartialMarkdown", () => {
  it("wraps heading1 content with # prefix", () => {
    ...
    assert.equal(
      ClipboardHandler.nodeToPartialMarkdown(node, "ading"),
      "# ading",
    );
  });
  ...
  it("wraps heading2 content with ## prefix", () => {
    ...
    assert.equal(
      ClipboardHandler.nodeToPartialMarkdown(node, "partial"),
      "## partial",
    );
  });
  ...
  it("returns paragraph content as-is", () => {
    ...
    assert.equal(
      ClipboardHandler.nodeToPartialMarkdown(node, "me tex"),
      "me tex",
    );
  });
  ...
  it("wraps blockquote content with > prefix", () => {
    ...
    assert.equal(
      ClipboardHandler.nodeToPartialMarkdown(node, "oted"),
      "> oted",
    );
  });
  ...
  it("wraps unordered list-item with marker", () => {
    ...
    assert.equal(
      ClipboardHandler.nodeToPartialMarkdown(node, "item"),
      "- item",
    );
  });
  ...
  it("wraps ordered list-item with number marker", () => {
    ...
    assert.equal(
      ClipboardHandler.nodeToPartialMarkdown(node, "text"),
      "3. text",
    );
  });
  ...
  it("wraps checklist item with checkbox prefix", () => {
    ...
    assert.equal(
      ClipboardHandler.nodeToPartialMarkdown(node, "todo"),
      "- [ ] todo",
    );
  });
  ...
  it("wraps indented list-item with indent spaces", () => {
    ...
    assert.equal(
      ClipboardHandler.nodeToPartialMarkdown(node, "nested"),
      "    - nested",
    );
  });
});
